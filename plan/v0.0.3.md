...
|-------|------|------|-------|
| `Xenova/all-MiniLM-L6-v2` | 384 | 23MB | Default, fast, no prefix |
| `Xenova/all-MiniLM-L12-v2` | 384 | 33MB | Better quality |
| `Xenova/bge-small-en-v1.5` | 384 | 33MB | State-of-art small |
...
// embeddings[0] = [0.023, -0.045, ...] (384 dimensions)
// embeddings[1] = [0.012, 0.067, ...] (384 dimensions)
```
...
CREATE TABLE code_chunks (
    ...
    embedding FLOAT[384]  -- 384 dimensions for all-MiniLM-L6-v2
);
```
...
SELECT 
    chunk_id,
    content,
    array_cosine_distance(embedding, $query_embedding::FLOAT[384]) AS distance
FROM code_chunks
ORDER BY array_cosine_distance(embedding, $query_embedding::FLOAT[384])
LIMIT 10;
```
...
- [ ] **1.2 Update DuckDB Service for VSS** (`src/services/vectorDbService.ts`)
  ...
  - [ ] Create `code_chunks` table with `FLOAT[384]` embedding column
